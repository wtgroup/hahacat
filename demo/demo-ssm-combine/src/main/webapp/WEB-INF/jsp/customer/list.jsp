...
<body>
				...
				<div class="panel-body">
					<form class="form-inline" action="${pageContext.request.contextPath }/customer/list.action" method="post">
						<div class="form-group">
							...
							<input type="text" class="form-control" id="customerName" value="${queryVo.customer.cust_name }" name="customer.cust_name">
						</div>
						<div class="form-group">
							<label for="customerFrom">客户来源</label> 
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="customer.cust_source">
								<option value="">--请选择--</option>
								<c:forEach items="${queryVo.fromType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == customer.cust_source}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							<label for="custIndustry">所属行业</label> 
							<select	class="form-control" id="custIndustry"  name="customer.cust_industry">
								<option value="">--请选择--</option>
								<c:forEach items="${queryVo.industryType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == customer.cust_industry}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							<label for="custLevel">客户级别</label>
							<select	class="form-control" id="custLevel" name="customer.cust_level">
								<option value="">--请选择--</option>
								<c:forEach items="${queryVo.levelType}" var="item">
									<option value="${item.dict_id}"<c:if test="${item.dict_id == customer.cust_level}"> selected</c:if>>${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
	...
	<script type="text/javascript">
		...
		function updateCustomer() {
			// $("#edit_customer_form").serialize() key1=value1&key2=value2...
